...
public class EmailService : IEmailService
{
    ...
    private async Task SendEmailAsync(string toEmail, string subject, string body)
    {
        ...
        try
        {
            ...
            _logger.LogDebug("SMTP client created. Connecting/authenticating to {Host}:{Port} (SSL={EnableSsl})", settings.SmtpHost, settings.SmtpPort, settings.EnableSsl);
        ...
        {
            _logger.LogError(ex, "Failed to send email to {Email} via {Host}:{Port}", toEmail, settings.SmtpHost, settings.SmtpPort);
            throw;
        }
    }
